...
function CalendarPage() {
  ...
  const events = useMemo(() => {
    const map = new Map<string, Array<{ id: string; title: string; tag: string; status: string; time?: string }>>();
    for (const i of feed) {
  ...
  const upcoming = useMemo(
    () =>
      feed
        .filter((i) => i.confirmed_time && new Date(i.confirmed_time) >= new Date(today.getFullYear(), today.getMonth(), today.getDate()))
    ...
    <AppShell>
      <header className="flex items-center justify-between px-5 pb-3 pt-8">
        <div>
          ...
          <p className="mt-0.5 text-sm text-ink-muted">Confirmed hangouts at a glance</p>
        </div>
      </header>
            ...
            return (
              <div
                ...
                className={`relative aspect-square rounded-lg p-1 text-[11px] transition ${
                  isToday ? "bg-teal text-primary-foreground" : dayEvents.length ? "bg-magenta-soft text-navy" : "text-ink"
                }`}
              >
